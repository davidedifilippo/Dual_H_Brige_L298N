...
const int pinVelocità = A0; //pin analogico deputato a leggere i valori del potenziometro
...
int velocità = 0;  //valore letto dal pin A0
...
void setup() {
  ...
  pinMode(pinVelocità, INPUT); //input in quanto legge il valore analogico del pin A0
...
void loop() {
  ...
  velocità = analogRead(pinAcceleratore);
  ...
  */
  potenza = map(duty_Cycle, 0, 1024, 0, 255);
